...
// либо массив (<id>, -<код ошибки>) в случае ошибки
    ...
    public static function sendSms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = false, $query = "", $files = array())
    {
        ...
        $m = self::smscSendCmd("send", "cost=3&phones=" . urlencode($phones) . "&mes=" . urlencode($message) .
...
// возвращает массив (<стоимость>, <количество sms>) либо массив (0, -<код ошибки>) в случае ошибки
    ...
    public static function getSmsCost($phones, $message, $translit = 0, $format = 0, $sender = false, $query = "")
    {
        ...
        $m = self::smscSendCmd("send", "cost=1&phones=" . urlencode($phones) . "&mes=" . urlencode($message) .
            ($sender === false ? "" : "&sender=" . urlencode($sender)) .
...
// либо массив (0, -<код ошибки>) в случае ошибки
    ...
    {
        $m = self::smscSendCmd("status", "phone=" . urlencode($phone) . "&id=" . urlencode($id) . "&all=" . (int)$all);
...
// возвращает баланс в виде строки или false в случае ошибки
    ...
    {
        $m = self::smscSendCmd("balance"); // (balance) или (0, -error)
...
// Функция вызова запроса. Формирует URL и делает 5 попыток чтения через разные подключения к сервису
    ...
    public static function smscSendCmd($cmd, $arg = "", $files = array())
    {
        ...
        do {
            ...
            $ret = self::smscReadUrl($url, $files, 3 + $i);
        } while ($ret == "" && $i < 5);
...
// curl или fsockopen (только http) или включена опция allow_url_fopen для file_get_contents
    ...
    public static function smscReadUrl($url, $files, $tm = 5)
    {
        $ret = "";
